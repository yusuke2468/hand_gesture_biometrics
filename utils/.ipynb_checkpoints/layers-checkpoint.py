...
class BasicConv(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1_7x7_s2 = nn.Conv2d(1, 64, kernel_size=(
...
class ECO_2D(nn.Module):
    ...
    def forward(self, x):
        '''
        入力xのサイズtorch.Size([batch_num, 1, 96, 96]))
        '''
...
class ECO_3D(nn.Module):
    def __init__(self):
        ...
        self.global_pool = nn.AvgPool3d(
            kernel_size=(8, 3, 3), stride=1, padding=0)

    def forward(self, x):
        '''
        入力xのサイズtorch.Size([batch_num,frames, 96, 12, 12]))
        '''
